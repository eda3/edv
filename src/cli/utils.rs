...
impl TimePosition {
    ...
    pub fn as_duration(&self) -> Duration {
        let secs = self.seconds.floor() as u64;
        let nanos = ((self.seconds - self.seconds.floor()) * 1_000_000_000.0) as u32;
...
/// Gets a list of media files in a directory.
///
/// # Arguments
///
/// * `dir` - The directory to search
/// * `extensions` - List of media file extensions to include (without the dot)
///
/// # Returns
///
/// A vector of paths to media files, or an error if the directory cannot be read.
///
/// # Errors
///
...
pub fn ensure_extension(file_name: &str, extension: &str) -> String {
    if file_name.to_lowercase().ends_with(&format!(".{extension}").to_lowercase()) {
        file_name.to_string()
    } else {
