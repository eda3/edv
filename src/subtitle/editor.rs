...
impl ShiftBuilder {
    ...
    fn should_shift_subtitle(&self, subtitle: &Subtitle) -> bool {
        // Check ID filter
        if !self.ids.is_empty() && !self.ids.iter().any(|id| id == subtitle.get_id()) {
            return false;
        }
...
impl SubtitleEditor {
    ...
    pub fn load_file<P: AsRef<Path>>(&mut self, path: P) -> Result<()> {
        ...
        // Parse the file
        let track = parser::parse_subtitle_file(path_ref, Some(format))?;
    ...
    pub fn merge_adjacent_subtitles(&mut self, max_gap: f64, same_style: bool) -> usize {
        ...
        while i < self.track.len().saturating_sub(1) {
            ...
            let can_merge =
                gap <= max_gap && (!same_style || current.get_style() == next.get_style());

            if can_merge {
                // Create a new merged subtitle
                let merged_text = format!("{}\n{}", current.get_text(), next.get_text());
                let merged = Subtitle::new(current.get_start(), next.get_end(), merged_text)
    ...
    pub fn split_subtitle(&mut self, id: &str, split_time: TimePosition) -> Result<bool> {
        ...
        if let Some(style) = subtitle.get_style().clone() {
            self.track
                .replace_subtitle(first_half.with_style(style.clone()));
    ...
    /// # Arguments
    ///
    /// * `strategy` - Strategy to use ("shorten" or "gap")
    /// * `min_gap` - Minimum gap between subtitles in seconds (for "gap" strategy)
    ///
    /// # Returns
    ///
...
mod tests {
    ...
    fn test_shift_subtitles() {
        ...
        assert_eq!(
            editor.get_subtitle("1").unwrap().get_start().as_seconds(),
            2.0
        );
        assert_eq!(
            editor.get_subtitle("2").unwrap().get_start().as_seconds(),
            5.0
        );
        assert_eq!(
            editor.get_subtitle("3").unwrap().get_start().as_seconds(),
            8.0
        );
    ...
    fn test_fix_overlaps() {
        ...
        editor
            .get_subtitle_mut("1")
            .unwrap()
            .set_end(TimePosition::from_seconds(4.0));
