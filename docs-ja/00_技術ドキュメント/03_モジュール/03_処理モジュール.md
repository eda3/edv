...
ビデオエフェクトとフィルターはビデオコンテンツの外観と品質を変更します。
...
マスキング機能も強化され、3つのタイプのマスクがサポートされるようになりました：

1. **シェイプマスク**: 矩形、楕円、ポリゴンなどの形状に基づいてコンテンツをマスクします
2. **アルファマスク**: 画像のアルファチャンネルを使用してマスクを作成します
3. **アニメーションマスク**: 時間の経過とともに変化するマスクを定義します

これらのマスクはブレンドモードと組み合わせて、強力な視覚効果を作成できます。 