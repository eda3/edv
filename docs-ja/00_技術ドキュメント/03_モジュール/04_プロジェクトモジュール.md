...
プロジェクトモジュールに、時間経過に伴ってプロパティ値を動的に変化させるキーフレームアニメーション機能が追加されました。この機能により、キーポイント間の値を補間してスムーズなトランジションやエフェクトを実現できます。

### 概要

キーフレームアニメーションの主な機能：
- 透明度、スケール、位置、回転などのプロパティをアニメーション化
- さまざまなイージング関数（線形、イーズイン、イーズアウトなど）のサポート
- キーフレーム間のプロパティ値補間
- すべてのキーフレーム操作に対する完全なUndo/Redoサポート

### 主要コンポーネント

#### キーフレームポイント

`KeyframePoint`は、特定の時間における値とイージング関数を表します：
...
`KeyframeTrack`は、単一のプロパティに対するキーフレームのシーケンスを管理します：
...
`KeyframeAnimation`クラスは、異なるプロパティの複数のトラックを管理します：
...
キーフレームアニメーションはタイムラインシステムと直接統合されています：

- 各`Track`はオプションで`KeyframeAnimation`を持つことができる
- アニメーションはトラックごとに適用され、クリッププロパティに影響する
- キーフレーム操作は履歴システムを通じて完全に元に戻すことが可能
- 任意の時点でプロパティ値を照会できる

### 使用例

```rust
// 透明度を0秒から10秒にかけて0.0から1.0にアニメーション化
...
キーフレームアニメーションシステムの計画中の改善点：
- 2D/3D座標のような多次元プロパティのサポート
- より精密な制御のためのベジェ曲線イージング関数
- 視覚的なキーフレーム編集ツール
- 一般的なエフェクト用のアニメーションプリセット 